...
namespace WebApplication6.Services
{
    public class YDService
    {
        ...
        public List<YD> GetDataList(string Search,string Searchradio)
        {
            ...
            {
                SQL = $@"SELECT * FROM GasList WHERE Pay LIKE '%{Searchradio}%' and (Gas NOT LIKE '% %');";
        ...
        public List<YD> GetKaoRecord(string Search, string Searchradio)
        {
            ...
            if (!string.IsNullOrWhiteSpace(Search) && !string.IsNullOrWhiteSpace(Searchradio))
            {
                ...
                SQL = $@"SELECT * FROM Record WHERE Pay LIKE '%{Searchradio}%' and (Gas LIKE '%{Search}%' OR Date LIKE '%{Search}%');";
            }
            //有字沒按鈕
            else if (!string.IsNullOrWhiteSpace(Search) && string.IsNullOrWhiteSpace(Searchradio))
            {
                SQL = $@"SELECT * FROM Record WHERE Gas LIKE '%{Search}%' OR Date LIKE '%{Search}%';";
            }
            //有按鈕沒字
            else if (string.IsNullOrWhiteSpace(Search) && !string.IsNullOrWhiteSpace(Searchradio))
            {
                SQL = $@"SELECT * FROM Record WHERE Pay LIKE '%{Searchradio}%' and (Gas NOT LIKE '% %');";
            ...
            else
            {
                ...
                string datetime = Year.ToString() + Month.ToString("00");
                SQL = $@" SELECT * FROM Record WHERE Date = '{datetime}';";
            }
        ...
        public void RecordInsert(string Gas,string Content, int Cost)
        {
            try
            {
                    ...
                    string datetime = Year.ToString() + Month.ToString("00");
                    conn.Open();
                    string SQL = $@"INSERT INTO Record(Gas,Content,Cost,Date) VALUES (N'{Gas}',N'{Content}','{Cost}','{datetime}') ; ";
                    SqlCommand cmd = new SqlCommand(SQL, conn);
                    cmd.ExecuteNonQuery();
        ...
        public void RecordUpdateData(YD updatedata)
        {
            string SQL = $@"UPDATE Record SET Gas = N'{updatedata.Gas}',Content= N'{updatedata.Content}',Pay= '{updatedata.Pay}',Cost= '{updatedata.Cost}' WHERE Id= '{updatedata.Id}'; ";
            try
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand(SQL, conn);
        ...
        public List<YD> KaoGetDataList(string Search, string Searchradio)
        {
            ...
            {
                SQL = $@"SELECT * FROM KaohsiungGasList WHERE Pay LIKE '%{Searchradio}%' and (Gas NOT LIKE '% %');";
            }
            else
            {
                DateTime date = DateTime.Today;
